import telebot
from datetime import datetime
import os
from dotenv import load_dotenv

load_dotenv()

TOKEN = os.getenv('TELEGRAM_BOT_TOKEN', '8583352280:AAEKrbrxN41W3557YhFuCRg37LQ_dvMkb1g')
ADMIN_IDS = [int(os.getenv('ADMIN_ID', '0'))]

bot = telebot.TeleBot(TOKEN)

@bot.message_handler(commands=['start'])
def send_welcome(message):
    user_id = message.from_user.id
    markup = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
    webApp = telebot.types.WebAppInfo(
        "https://ppl-ai-code-interpreter-files.s3.amazonaws.com/web/direct-files/1cba166588e51af668d0e7b4e5d393ef/3204afed-0e38-40e7-bd9a-7a0c3ae76af1/index.html"
    )
    markup.add(telebot.types.KeyboardButton(text="🚀 Открыть Chain Pro", web_app=webApp))
    welcome_text = """
🎯 **Добро пожаловать в Legion Chain Score Pro!**

Вот что ты можешь делать:
✅ Автоматически повышай Chain Score
✅ Выбери одну из 3 подписок
✅ Поддержка 3 блокчейнов (Arbitrum, Solana, Sui)
✅ Отслеживай все транзакции в реальном времени
✅ Получай оценки улучшения Chain Score

Нажми кнопку ниже, чтобы начать!
    """
    bot.send_message(message.chat.id, welcome_text, reply_markup=markup, parse_mode='Markdown')

@bot.message_handler(commands=['info'])
def send_info(message):
    info_text = """
💎 **LEGION CHAIN PRO - ПОДПИСКИ**

📌 **STARTER** - 700 ⭐
├─ Длительность: 7 дней
├─ Транзакции: 120-150
├─ Chain Score: +142 баллов
└─ Блокчейн: 1 на выбор

📌 **PROFESSIONAL** - 1000 ⭐ (РЕКОМЕНДУЕТСЯ)
├─ Длительность: 14 дней
├─ Транзакции: 350
├─ Chain Score: +400 баллов
└─ Блокчейны: Все 3

📌 **ENTERPRISE** - 1500 ⭐
├─ Длительность: 30 дней
├─ Транзакции: 1000+
├─ Chain Score: +750 баллов
└─ Блокчейны: Все 3 + Мультикошельки

🔗 Открой приложение, чтобы выбрать подписку!
    """
    bot.send_message(message.chat.id, info_text, parse_mode='Markdown')

@bot.message_handler(commands=['help'])
def send_help(message):
    help_text = """
📖 **ДОСТУПНЫЕ КОМАНДЫ**

/start - Открыть Legion Chain Pro
/info - Просмотреть подписки
/help - Эта справка
/status - Проверить статус подписки
/support - Получить поддержку
/admin - Админ-панель (только для администраторов)

💬 Вопросы? Используй команду /support
    """
    bot.send_message(message.chat.id, help_text, parse_mode='Markdown')

@bot.message_handler(commands=['status'])
def check_status(message):
    user_id = message.from_user.id
    status_text = f"""
👤 **Статус твоей подписки**

ID пользователя: {user_id}
Статус: Нет активной подписки
План: Отсутствует
Дней осталось: -

💡 Чтобы начать:
1. Нажми /start
2. Откроется мини-приложение
3. Выбери подписку
4. Оплати Telegram Stars
5. Начни автоматизацию!
    """
    bot.send_message(message.chat.id, status_text, parse_mode='Markdown')

@bot.message_handler(commands=['support'])
def send_support(message):
    support_text = """
🆘 **СЛУЖБА ПОДДЕРЖКИ**

Для помощи:
📧 Email: support@legion.cc
🌐 Сайт: https://legion.cc
💬 Telegram: @LegionSupport

Частые вопросы:
❓ Платёж не прошёл? Проверь баланс Telegram Stars
❓ Автоматизация не запускается? Подожди 1-2 минуты после оплаты
❓ Нужна админ-роль? Свяжись с командой поддержки
    """
    bot.send_message(message.chat.id, support_text, parse_mode='Markdown')

@bot.message_handler(commands=['admin'])
def admin_panel(message):
    user_id = message.from_user.id
    if user_id not in ADMIN_IDS:
        bot.send_message(message.chat.id, "❌ У тебя нет доступа к админ-панели.")
        return
    admin_text = """
🔐 **АДМИН-ПАНЕЛЬ**

Доступные функции:
1. ✅ Управление пользователями
2. ✅ Управление белым списком
3. ✅ Статистика и аналитика
4. ✅ Настройки системы

📌 Примечание: Все действия администратора логируются для безопасности.

Откройте мини-приложение Legion Chain Pro, чтобы получить доступ к полной админ-панели.
    """
    bot.send_message(message.chat.id, admin_text, parse_mode='Markdown')

@bot.message_handler(func=lambda message: True)
def handle_text(message):
    response = """
👋 Привет! Я Legion Chain Pro Bot.

Доступные команды:
/start - Открыть приложение
/info - Информация о подписках
/help - Справка
/support - Поддержка

Нажми /start, чтобы начать!
    """
    bot.send_message(message.chat.id, response)

if __name__ == '__main__':
    print("=" * 60)
    print("LEGION CHAIN PRO - TELEGRAM BOT (HEROKU)")
    print("=" * 60)
    print(f"🚀 Бот запущен в {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("📡 Слушаю входящие сообщения...")
    print("=" * 60)
    print()
    print("Новые цены подписок:")
    print("  STARTER: 700 ⭐ (7 дней)")
    print("  PROFESSIONAL: 1000 ⭐ (14 дней)")
    print("  ENTERPRISE: 1500 ⭐ (30 дней)")
    print()
    print("Команды:")
    print("  /start - Открыть приложение")
    print("  /info - Информация о подписках")
    print("  /help - Справка")
    print("  /admin - Админ-панель")
    print("=" * 60)
    
    try:
        bot.polling()
    except Exception as e:
        print(f"Ошибка: {e}")
        print("Перезагрузка бота...")
